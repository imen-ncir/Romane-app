...
interface UpdateFlashcardProps extends BaseScreenProps {}

export const UpdateFlashcard = ({navigation, route}: UpdateFlashcardProps) => {
  ...
  const {updateFlashcard} = useRecoilFlashcards();
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const response = await FlashcardApi.getFlashcard(id);
      if (response.isRight() && isSubscribed) {
        const flashcard: FlashcardDetailsDTO = response.value.getValue();
        const {question, answer} = flashcard;
        setQuestion({
          audioFileUri: question.audioUrl,
          text: question.text,
          pictureFileUri: question.pictureUrl,
        });
        ...
        setAnswer({
          type: type,
          textValue:
            type === FlashcardTypes.TEXT
              ? (value as AnswerTextDTO).text
              : undefined,
          flashcardValue:
            type === FlashcardTypes.FLASHCARD
              ? {
                  text: (value as AnswerTextAudioDTO).text,
                  audioFileUri: (value as AnswerTextAudioDTO).audioUrl,
                }
              : undefined,
          multipleValues:
            type === FlashcardTypes.MULTIPLE
              ? (value as AnswerMultipleTextAudioDTO[]).map(v => ({
                  text: v.text,
                  audioFileUri: v.audioUrl,
                  isRight: v.isRight,
                }))
              : undefined,
        });
  ...
  const handleSave = async (
    ...
    const response = await FlashcardApi.updateFlashcard(id, payload);
    setLoading(false);
    if (response.isRight()) {
      // Api Result
